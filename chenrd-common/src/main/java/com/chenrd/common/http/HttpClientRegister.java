...
public class HttpClientRegister
{
    ...
    private Map<String, String> nextReqHeads = null;
    ...
    {
        if (nextReqHeads == null) nextReqHeads = new HashMap<String, String>();
        nextReqHeads.put(name, val);
    }
    
    public void addAllReqHead(Map<String, String> heads)
    {
        if (nextReqHeads == null) nextReqHeads = new HashMap<String, String>();
    ...
    public HttpClientRegister(String realmName, int type) throws GeneralSecurityException
    {
        ...
        if (type == TYPE_BASE)
        {
            ...
            RequestConfig defaultRequestConfig = RequestConfig.custom().setSocketTimeout(10000).setConnectTimeout(6000).setConnectionRequestTimeout(6000)
    ...
    private CallResult call(HttpRequestBase request, Map<String, Object> params, RequestParamType type) throws HttpFailException
    {
        ...
        try {
            ...
            if (nextReqHeads != null && nextReqHeads.size() > 0) {
                for (Entry<String, String> entry : nextReqHeads.entrySet())
                    request.setHeader(entry.getKey(), entry.getValue());
                nextReqHeads.clear();
            }
            request.setConfig(RequestConfig.custom().setSocketTimeout(6000).setConnectTimeout(6000).build());
            response = client.execute(request);
            int status = response.getStatusLine().getStatusCode();
            if (status != HttpStatus.SC_OK
                && status != HttpStatus.SC_MOVED_TEMPORARILY && status != HttpStatus.SC_MOVED_PERMANENTLY)
